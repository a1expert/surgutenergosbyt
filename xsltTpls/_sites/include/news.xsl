...
<xsl:stylesheet	version="1.0"
	...
	<xsl:template match="item" mode="news_full">
		<xsl:variable name="source_url" select="document(concat('upage://', @id, '.source'))//value" />
    	...
    	<div class="news">
			...
			<p class="link"><a href="{$source_url}" target="_blank"><xsl:value-of select="document(concat('upage://', @id, '.h1'))//value" /></a></p>
	...
	<xsl:template match="item" mode="news1">
		<!--<xsl:variable name="source" select="document(concat('upage://', @id, '.source_url'))//value" />-->
		<xsl:variable name="source_url" select="document(concat('upage://', @id, '.source'))//value" />
		...
		<div class="tumb">
			...
			<p class="link"><a href="{$source_url}" target="_blank"><xsl:value-of select="substring($title_cut, 1, 100)" />&nbsp;<xsl:text>...</xsl:text></a></p>
            <!--<p class="link"><a href="{@link}"><xsl:value-of select="substring($title_cut, 1, 100)" /></a></p>-->
		</div>
	</xsl:template>

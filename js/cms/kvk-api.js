	...
	this.cancel = function() {
		...
		openDialog({
			...
			text: getLabel('js-label-cancel_form') + "<p></p>" +
				...
				"<input type='radio' name='reason' value='Отказ клиента' checked='checked' /> " + getLabel('js-label-cancel_form-client') + "<br/>" +
				"<input type='radio' name='reason' value='Отказ магазина' /> " + getLabel('js-label-cancel_form-partner') + "<br/>" +
				"<input type='radio' name='reason' value='Отсутствует товар' /> " + getLabel('js-label-cancel_form-good-missed') + "<br/>" +
				"<input type='radio' name='reason' value='Ошибочная заявка' /> " + getLabel('js-label-cancel_form-wrong') + "<br/>" +
				"<input type='radio' name='reason' value='Клиент неконтактен' /> " + getLabel('js-label-cancel_form-no-contact') + "<br/>" +
				"<input type='radio' name='reason' value='Несоотв. данных паспорта и договора' /> " + getLabel('js-label-cancel_form-fail-offer') +
				"</form>",
			OKCallback: function() {
				self.loadingMessage();

	...
	class quickCsvImporter implements iQuickCsvImporter {
				...
				if ($error) {
					...
					if (!($file instanceof iUmiFile) || $file->getIsBroken()) {
						...
						foreach ($sel->types as $type) {
							if (!is_null($type->objectType)) {
								$objectTypes[] = $type->objectType;
							}
							if (!is_null($type->hierarchyType)) {
								$hierarchyTypes[] = $type->hierarchyType;
							}
						}
		...
		protected function importObjects(umiObjectType $objectType) {
			...
			while ($cols = $this->readNextRow()) {
				...
				/*
				 * TODO: Это пиздец кривой костыль для обхода проверки пароля...
				 * Надо переделать, причем переделывать надо всю модель импорта и валидации
				 * точнее создавать модель..
				 */

 ...
 */
	abstract class __client_reflection_data {
		...
		public function saveEditedObject($object_id, $is_new = false, $b_force_owner = false, $all = false) {
			...
			if ((!$permissions->isSv()) && ($object->getType()->getGUID() == 'users-user' || $object->getType()->getGUID() == 'emarket-customer')) {
				...
				$notAllowedFields = array('filemanager_directory', 'bonus', 'spent_bonus'); 
				foreach($notAllowedFields as $notAllowedField) {
					if (isset($data[$notAllowedField])) {
						unset($data[$notAllowedField]);
				}
			}
			}

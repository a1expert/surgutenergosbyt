			...
			if($has_value || self::$showEmptyFields || translatorWrapper::$showEmptyFields) {
				...
				if ($fieldDataType == 'string') {
					...
					if($restriction instanceof baseRestriction) {
						$resultArray['@restriction'] = $restriction->getClassName();
					}
				}
				...
				switch($fieldDataType) {
					...
					case "img_file": {
						...
						if ($value->getIsBroken()) {
							$resultArray['value']['@path'] = '.' . $path;
							$resultArray['value']['@is_broken'] = 1;
							$resultArray['value']['#value'] = $path;
							break;
						}

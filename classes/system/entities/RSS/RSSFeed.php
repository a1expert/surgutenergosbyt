<?php
	class RSSFeed implements iRSSFeed {
		...
		public function loadContent($charset = false) {
			...
			if (strpos(strtolower($headers), 'content-encoding: gzip')) {
				$cont = gzinflate(substr($cont, 10));
			}
			...
			if ($charset) {
				$cont = iconv(strtoupper($charset), "UTF-8//IGNORE", $cont);
		...
		public function error_handler($errno, $errstr) {
			if (strpos($errstr, 'Detected an illegal character in input string')) {
		...
		public function loadAtom() {
			if (strtolower($this->xml->getName()) != 'feed') {
				throw new publicAdminException(getLabel('label-feed-incorrect-type', false, $this->url));
			}

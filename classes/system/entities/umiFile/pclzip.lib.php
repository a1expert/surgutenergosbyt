    ...
    if ($v_size > 0) {
      ...
      if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
        ...
        if (isset($v_options[PCLZIP_OPT_ADD_PATH])) {
          ...
          if ((string_bytes($v_path) > 0) && (substr($v_path, -1) != '/')) {
            $v_path .= '/';
          }
...
//          || (   is_dir($p_filedescr_list[$j]['filename'])
  ...
  function privAddFile($p_filedescr, &$p_header, &$p_options)
  {
    ...
    */
    ...
    $p_header['filename_len'] = string_bytes($p_filename);
...
// TBC : Removed    $p_header['stored_filename'] = $v_stored_filename;
    ...
    // ----- Look for virtual file
      ...
      $p_header['external'] = 0x00000000;
      $p_header['size'] = string_bytes($p_filedescr['content']);
    }
    ...
    if (isset($p_filedescr['comment'])) {
      $p_header['comment_len'] = string_bytes($p_filedescr['comment']);
    ...
    if (string_bytes($p_header['stored_filename']) > 0xFF) {
...
//      if (is_file($p_filename))
      if (   ($p_filedescr['type'] == 'file')
        ...
        // ----- Look for normal compression
          ...
          // ----- Set header parameters
          $p_header['compressed_size'] = string_bytes($v_content);
        ...
        if ((isset($p_options[PCLZIP_OPT_CRYPT]))
          ...
          //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Size after header : ".string_bytes($v_content_compressed)."");
        }
        */
  ...
  function privCalculateStoredFilename(&$p_filedescr, &$p_options)
  {
    ...
    //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Stored filename will be '".$p_filedescr['stored_filename']."', string_bytes ".string_bytes($p_filedescr['stored_filename']));
  ...
  function privWriteFileHeader(&$p_header)
  {
    ...
    $v_binary_data = pack("VvvvvvVVVvv", 0x04034b50,
						  ...
						  $p_header['size'],
                          string_bytes($p_header['stored_filename']),
						  $p_header['extra_len']);
    ...
    if (string_bytes($p_header['stored_filename']) != 0)
    {
      fputs($this->zip_fd, $p_header['stored_filename'], string_bytes($p_header['stored_filename']));
    }
  ...
  function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
  {
    ...
    $v_binary_data = pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries,
	                      $p_nb_entries, $p_size,
						  $p_offset, string_bytes($p_comment));

    // ----- Write the 22 bytes of the header in the zip file
    fputs($this->zip_fd, $v_binary_data, 22);

    // ----- Write the variable fields
    if (string_bytes($p_comment) != 0)
    {
      fputs($this->zip_fd, $p_comment, string_bytes($p_comment));
    }
  ...
  function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
    ...
    if (($p_path != "./") && ($p_path != "/"))
    {
      ...
      while (substr($p_path, -1) == "/")
      {
        ...
        $p_path = substr($p_path, 0, string_bytes($p_path)-1);
    ...
    $p_remove_path_size = string_bytes($p_remove_path);
      ...
      if (   (isset($p_options[PCLZIP_OPT_BY_NAME]))
          ...
          for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
              ...
              if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                  ...
                  if (   (string_bytes($v_header['stored_filename']) > string_bytes($p_options[PCLZIP_OPT_BY_NAME][$j]))
                      && (substr($v_header['stored_filename'], 0, string_bytes($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
...
//      if ($p_entry['compressed_size'] == $p_entry['size'])
  ...
  function privReadFileHeader(&$p_header)
  {
    ...
    if (string_bytes($v_binary_data) != 26)
    {
      $p_header['filename'] = "";
      $p_header['status'] = "invalid_header";
      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Invalid block size : ".string_bytes($v_binary_data));

      // ----- Error log
      PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid block size : ".string_bytes($v_binary_data));
  ...
  function privReadCentralFileHeader(&$p_header)
  {
    ...
    // ----- Look for invalid block size
    if (string_bytes($v_binary_data) != 42)
    {
      $p_header['filename'] = "";
      $p_header['status'] = "invalid_header";
      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Invalid block size : ".string_bytes($v_binary_data));
  ...
  function privReadEndCentralDir(&$p_central_dir)
  {
    ...
    // ----- Look for invalid block size
    if (string_bytes($v_binary_data) != 18)
    {
      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 2, "Invalid End of Central Dir Record size : ".string_bytes($v_binary_data));

      // ----- Error log
      PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid End of Central Dir Record size : ".string_bytes($v_binary_data));
      ...
      if (   (isset($p_options[PCLZIP_OPT_BY_NAME]))
          ...
          for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
              ...
              if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                  ...
                  // ----- Look if the directory is in the filename path
                  if (   (string_bytes($v_header_list[$v_nb_extracted]['stored_filename']) > string_bytes($p_options[PCLZIP_OPT_BY_NAME][$j]))
                      && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, string_bytes($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                      //--(MAGIC-PclTrace)--//PclTraceFctMessage(__FILE__, __LINE__, 3, "The directory is in the file path");
                      $v_found = true;
                  }
  ...
  function privDirCheck($p_dir, $p_is_dir=false)
  {
    ...
    {
      $p_dir = substr($p_dir, 0, string_bytes($p_dir)-1);
    }

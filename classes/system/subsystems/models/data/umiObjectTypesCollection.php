 ...
 */
	class umiObjectTypesCollection extends singleton implements iSingleton, iUmiObjectTypesCollection {
		private $types = Array();
		private $typesGuidsToId = array();

		/**
		 * Конструктор
		 */
		protected function __construct() {
			$this->preLoadTypesIdsByGuid();
		}
		...
		public function getTypeIdByGUID($guid) {
			...
			if (isset($this->typesGuidsToId[$guid])) {
				return $cache[$guid] = $this->typesGuidsToId[$guid];
			}
			...
			if(list($id) = mysql_fetch_row($result)) {
				return $this->typesGuidsToId[$guid] = $cache[$guid] = $id;
			} else {
				return $this->typesGuidsToId[$guid] = $cache[$guid] = false;
			}
		}
		 ...
		 * @param Boolean $ignore_parent_groups
		...
		public function getChildClasses($type_id, $childs = false) {
			...
			while (list($id) = mysql_fetch_row($result)) {
				...
				if (!in_array($id, $childs)) {
					$res = array_merge($res, $this->getChildClasses($id, $res));
				}
			}
		...
		public function getTypesByHierarchyTypeId($hierarchy_type_id, $ignoreMicroCache = false) {
			...
			if (isset($cache[$hierarchy_type_id]) && $ignoreMicroCache == false) {
				return $cache[$hierarchy_type_id];
			}
		...
		public function getTypeByHierarchyTypeId($hierarchy_type_id, $ignoreMicroCache = false) {
			...
			if (isset($cache[$hierarchy_type_id]) && $ignoreMicroCache == false && cmsController::$IGNORE_MICROCACHE == false) {
				return $cache[$hierarchy_type_id];
			}
		...
		public function getHierarchyTypeIdByObjectTypeId($objectTypeId) {
			...
			if ($row && isset($row['id'])) {
				$hierarchyTypeId = $row['id'];
			} else {
				$hierarchyTypeId = false;
			}

			return $cache[$objectTypeId] = $hierarchyTypeId;
		}
		 ...
		 * @throws coreException
		 */
		private function preLoadTypesIdsByGuid() {
			$mainConfigs = mainConfiguration::getInstance();
			$guids = $mainConfigs->get('kernel', 'objects-types-guids-preload');
			if (!is_array($guids) || count($guids) == 0) {
				return;
			}
			$guids = array_map('l_mysql_real_escape_string', $guids);
			$sql = "SELECT `id`, `guid` FROM `cms3_object_types` WHERE `guid` in ('" . implode('\', \'', $guids) . "');";
			...
			$typesGuidsToId = &$this->typesGuidsToId;

			while ($row = mysql_fetch_assoc($result)) {
				$typesGuidsToId[$row['guid']] = $row['id'];
			}
		}
	}
?>
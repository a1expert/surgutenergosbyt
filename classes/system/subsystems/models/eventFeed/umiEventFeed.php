<?php
	class umiEventFeed {
		...
		*/
		public static function create(umiEventFeedType $type, $params = array(), $elementId = null, $objectId = null) {
			...
			self::getConnection()->query("INSERT INTO `umi_event_feeds` (type_id, date, params, element_id, object_id) VALUES('{$typeId}', '{$date}', '{$params}', '{$elementId}', '{$objectId}')");
		...
		*/
		public static function markReadEvent($eventId, $userId) {
			$eventId = (int) $eventId;
			$userId = (int) $userId;
			self::getConnection()->query("INSERT INTO `umi_event_user_history` (user_id, event_id) VALUES('{$userId}', '{$eventId}')");
		}
		...
		*/
		public static function markUnreadEvent($eventId, $userId) {
			$eventId = (int) $eventId;
			$userId = (int) $userId;
			self::getConnection()->query("DELETE FROM `umi_event_user_history` WHERE user_id = '{$userId}' AND event_id = '{$eventId}'");
		}
		...
		*/
		public function findEventIdByElementId($elementId) {
			...
			$events = self::getConnection()->queryResult("SELECT id FROM `umi_event_feeds` WHERE element_id = {$elementId}");
		...
		*/
		public function findEventIdByObjectId($objectId) {
			...
			$events = self::getConnection()->queryResult("SELECT id FROM `umi_event_feeds` WHERE object_id = {$objectId}");
			foreach($events as $event) {
				$eventId = $event['id'];
			}
		...
		*/
		public static function getList($types = array(), $userId, $limit = null, $offset = null, $startDate = null, $endDate = null) {
			...
			$list = array();
			$sql = "SELECT @a:=(SELECT `read` FROM `umi_event_user_history` WHERE `event_id` = `id` AND `user_id` = {$userId} GROUP BY event_id) as `read`, `id` FROM `umi_event_feeds` WHERE";
			...
			if ($startDate) $sql .= " `date` > {$startDate} AND";
			if ($endDate) $sql .= " `date` < {$endDate} AND";
		...
		*/
		public static function getUnreadList($typeId, $userId, $limit = null, $offset = null, $startDate = null, $endDate = null) {
			$limit = (int) $limit;
			$offset = (int) $offset;
			$startDate = (int) $startDate;
			$endDate = (int) $endDate;
		...
		*/
		public static function deleteList($types = array(), $startDate = null, $endDate = null) {
		...
		* @return int
		*/
		public static function getListCount($types = array(), $userId = null, $startDate = null, $endDate = null) {
			
			$startDate = (int) $startDate;
			$endDate = (int) $endDate;
			$userId = (int) $userId;
			
			$sql = "SELECT count(*)	FROM `umi_event_feeds` WHERE";
			if (is_array($types) && count($types)) {
				foreach ($types as &$typeId) {
					$typeId = self::getConnection()->escape($typeId);
				}
		...
		*/
		public function load() {
			$id = (int) $this->id;
			$eventInfo = self::getConnection()->queryResult("SELECT type_id, date, params FROM umi_event_feeds WHERE id = {$id}", true);

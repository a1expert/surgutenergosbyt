	...
	class umiEventFeedUser {
		...
		*/
		public static function create($id) {
			...
			self::getConnection()->queryResult("INSERT INTO umi_event_users (id, last_check_in, settings) VALUES('{$id}', '{$time}', '{$settings}')");
			$user = new self($id);
			self::$users[$id] = $user;
			return $user;
		}
		...
		*/
		protected function load() {
			$id = (int) $this->id;
			$userInfo = self::getConnection()->queryResult("SELECT last_check_in, settings FROM umi_event_users WHERE id = {$id}", true);
		...
		*/
		public function save() {
			...
			$id = (int) $this->id;
			$lastCheckIn = $this->lastCheckIn ? $this->lastCheckIn : time();	
			$settings = serialize($this->settings);
			self::getConnection()->query("UPDATE umi_event_users SET last_check_in = '{$lastCheckIn}', settings = '{$settings}' WHERE id = {$id}");

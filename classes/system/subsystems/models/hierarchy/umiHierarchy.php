	...
	class umiHierarchy extends singleton implements iSingleton, iUmiHierarchy {
		...
		*/
		public function isExists($elementId) {
			...
			if (permissionsCollection::getInstance()->isPageReadable($elementId)) {
				return true;
			}
		...
		*/
		public function getElement($element_id, $ignorePermissions = false, $ignoreDeleted = false, $row = false) {
			...
			if(!$ignorePermissions && !$this->isAllowed($element_id)){
				return false;
			}

			$cacheFrontend = $this->cacheFrontend;
		...
		*/
		public function delElement($element_id) {
			...
			if($element = $this->getElement($element_id)) {
				...
				$event->setParam('user_id', $permissions->getUserId());
				$event->setMode("after");
				$event->call();
...
SQL;
				...
				$object_type = $this->umiObjectsCollection->getObject($object_id)->getTypeId();
...
TOTALSQL;
		...
		*/
		public function restoreElement($element_id) {
			...
			if($element = $this->getElement($element_id, false, true)) {
				...
				$event->setParam('user_id', permissionsCollection::getInstance()->getUserId());
		...
		*/
		...
		/**
		 * Получить адрес страницы по ее идентификатору
		 * @param int $elementId страница, путь до которой нужно получить
		 * @param bool $ignoreLang игнорировать языковой префикс к адресу страницы
		 * @param bool $ignoreIsDefaultStatus игнорировать статус страницы "по умолчанию" и сформировать для не полный путь
		...
		*/
		public function getPathById($elementId, $ignoreLang = false, $ignoreIsDefaultStatus = false, $ignoreCache = false, $ignoreUrlSuffix = false) {
			...
			$cachePath = $elementId . '#' . intval($ignoreLang) . intval($ignoreIsDefaultStatus) . intval($ignoreUrlSuffix) . '#' . intval($this->bForceAbsolutePath);
			...
			if ($element = $this->getElement($elementId, true)) {
				...
				$elementLang = $langs->getLang($elementLangId);
		...
		*/
		public function addElement($rel_id, $hierarchy_type_id, $name, $alt_name, $type_id = false, $domain_id = false, $lang_id = false, $tpl_id = false) {
			...
			$hierarchy_type = null;		
			if($type_id === false) {
				if($hierarchy_type = $this->umiHierarchyTypesCollection->getType($hierarchy_type_id)) {
				...
				$object_type = $this->umiObjectTypesCollection->getType($type_id);
				if (!$object_type) {
			...
			if (!$hierarchy_type instanceof umiHierarchyType) {
			...
			if(!$tpl_id) {
				$tpl_id = $this->templatesCollection->getHierarchyTypeTemplate($hierarchy_type->getName(), $hierarchy_type->getExt());
				if($tpl_id === false) {
					$tpl_id = $this->getDominantTplId($rel_id);
					if(!$tpl_id) {
						$tpl = $this->templatesCollection->getDefaultTemplate($domain_id, $lang_id);
			...
			if($object_id = $this->umiObjectsCollection->addObject($name, $type_id)) {
		...
		*/
		...
		/**
		 * Включен ли режим генерации абсолютных url'ов
		 * @return bool
		 */
		public function isPathAbsolute() {
			return (bool) $this->bForceAbsolutePath;
		}
		...
		*/
		public function moveBefore($element_id, $rel_id, $before_id = false) {
			...
			$arrTpls = $this->templatesCollection->getTemplatesList($domain_id, $lang_id);
			$bNeedChangeTpl = true;
			...
			if ($bNeedChangeTpl) {
				$oDefaultTpl = $this->templatesCollection->getDefaultTemplate($domain_id, $lang_id);
				if ($oDefaultTpl) {
		...
		*/
		public function isAllowed($elementId) {
			...
			$readStatus = permissionsCollection::getInstance()->isPageReadable($elementId);
			return $readStatus[0];
		}
...
SQL;
			...
			if ($type_id = (int) $this->cacheFrontend->loadSql($sql)) {
				return $type_id;
			}
            ...
            $this->cacheFrontend->saveSql($sql, $type_id);
		...
		*/
		public function hasParent($hierarchy, $hierarchy_parent) {
			...
			if(is_numeric($hierarchy)) {
				$hierarchy = $this->getElement ($hierarchy);
			}

			if(is_numeric($hierarchy_parent)) {
				$hierarchy_parent = $this->getElement ($hierarchy_parent);
			}
		...
		public function getRightAltName($alt_name, $element, $b_fill_cavities = false, $ignore_cur_element = false) {
			...
			if ($element->getRel() == 0 && !IGNORE_MODULE_NAMES_OVERWRITE) {
				...
				$modules_keys = $this->regedit->getList("//modules");
				...
				if ($this->langsCollection->getLangId($alt_name)) {
					$alt_name .= '1';
				}
			}

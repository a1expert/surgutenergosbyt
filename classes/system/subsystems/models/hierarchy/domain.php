...
*/
	class domain extends umiEntinty implements iUmiEntinty, iDomainMirrow, iDomain {
		...
		*/
		...
		/**
		* Возврашает имя текущего домена или зеркала домена, если мы находимся на нем.
		* 
		*/
		public function getCurrentHostName() {
			$httpHost = getServer('HTTP_HOST');

			if (strpos($httpHost, 'xn--') === 0) {
				$idnaConvert = new idna_convert();
				$httpHost = $idnaConvert->decode($httpHost);
			}

			if ($this->host != $httpHost) {
				foreach($this->mirrows as $mirrow) {
					if($mirrow->getHost() == $httpHost) {
						return $mirrow->getHost();
					}
				}
			}

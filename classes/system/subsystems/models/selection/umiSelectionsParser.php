	...
	class umiSelectionsParser implements iUmiSelectionsParser {
		...
		*/
		public static function runSelection(umiSelection $selection) {
            ...
            $notOnlyIdsTaken = $selection->needSimplePageFields;

            if ($notOnlyIdsTaken) {
                $pageIds = array();
                while ($row = mysql_fetch_assoc($result)) {
                    if (count($row) === 4 && $permissions->isPageReadable($row['id'])) {
                        $res[] = $row;
                        $pageIds[] = $row['id'];
                    }
                }
                $umiLinksHelper = umiLinksHelper::getInstance();
                $umiLinksHelper->loadLinkPartForPages($pageIds);
            } else {
			...
			if($selection->excludeNestedPages) {
				$res = self::excludeNestedPages($res, $notOnlyIdsTaken);
			}
		...
		*/
		...
		private static function makeSelectExpr(umiSelection $selection) {
			if ($selection->sql_cond__need_hierarchy) {
                ...
                if ($selection->needSimplePageFields) {
                    $selection->sql_select_expr = "h.id, h.alt_name, h.rel, o.name";
                }
		...
		private static function makeTables(umiSelection $selection) {
			...
			if ($selection->sql_cond__need_hierarchy) {
				if($selection->sql_part__content_tables || $selection->objectTableIsRequired || $selection->needSimplePageFields) {
...
SQL;
		...
		protected static function excludeNestedPages($arr, $onlyIds = true) {

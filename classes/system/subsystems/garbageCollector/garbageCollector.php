	...
	class garbageCollector implements iGarbageCollector {
		...
		*/
		protected function deleteBrokenDBRelations() {
			...
			$this->deleteBrokenForeignRelations("cms3_objects", "type_id", "cms3_object_types", "id"); // Удаление объектов несуществующих типов
			$this->deleteBrokenForeignRelations("cms3_hierarchy", "obj_id", "cms3_objects", "id"); // Удаление страниц иерархии, которые ссылаются на несуществующие объекты
			$this->deleteBrokenForeignRelations("cms3_hierarchy", "rel", "cms3_hierarchy", "id"); // Удаление страниц иерархии, у которых отсутствуют родители
		}
		...
		*/
		protected function deleteBrokenForeignRelations($leftTableName, $leftColumnName, $rightTableName, $rightColumnName) {
			...
			$leftTableName = l_mysql_real_escape_string($leftTableName);
			$leftColumnName = l_mysql_real_escape_string($leftColumnName);
			$rightTableName = l_mysql_real_escape_string($rightTableName);
			$rightColumnName = l_mysql_real_escape_string($rightColumnName);

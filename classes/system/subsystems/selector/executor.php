<?php
	class selectorExecutor {
		...
		public function result() {
			...
			if (!DISABLE_CALC_FOUND_ROWS && !$this->selector->option('no-length')->value) {
			...
			if ($this->selector->mode == 'objects') {
				...
				if (count($objectIds) > 0 && $this->selector->option('load-all-props')->value && $this->hierarchyTypesCount == 1) {
					umiObjectProperty::loadPropsData($objectIds, $this->hierarchyTypesCount);
				}
				return def_module::sortObjects($objectIds, $list);
			} else {
				$ids = array();
				...
				if(count($listIds)) {
					...
					if ($this->selector->option('no-permissions')->value) {
						...
						foreach($listIds as $id) {
							if ($hierarchy->isAllowed($id)) {
								$permittedIds[$id];
							}
						}
						$listIds = $permittedIds;
						if (count($listIds) == 0) {
							return $outputList;
						}
					}
					$umiLinksHelper = umiLinksHelper::getInstance();
					$umiLinksHelper->loadLinkPartForPages($listIds);
					$sql = "SELECT h.id, h.rel, h.type_id, h.lang_id, h.domain_id, h.tpl_id, h.obj_id, h.ord, h.alt_name, h.is_active, h.is_visible, h.is_deleted, h.updatetime, h.is_default, o.name, o.type_id as object_type_id FROM cms3_hierarchy h, cms3_objects o WHERE h.id IN (" . implode(',', $listIds) . ") AND o.id = h.obj_id;";
					...
					foreach($result as $row) {
						...
						if (sizeof($return)) {
							if (sizeof($return) == 1 && $return[0] == 'id') {
								$outputList[$elementId] = array('id' => $elementId);
							}
							else {
								...
								if (!isset($list_items['id'])) {
									$list_items['id'] = $elementId;
								}
								$outputList[$elementId] = $list_items;
								$objectIds[] = $element->getObjectId();
							}
						}
						else {
							$element = $hierarchy->getElement($elementId, false, false, $row);
							if($element instanceof iUmiHierarchyElement) {
								$outputList[$elementId] = $element;
								$objectIds[] = $element->getObjectId();
							}
						}
					}
				...
				return def_module::sortObjects($listIds, $outputList);
			}
		}
		...
		public static function getContentTableName(selector $selector, $fieldId) {
			...
			foreach($types as $type) {
				if(is_null($type->objectTypeIds) == false) $objectTypes = $type->objectTypeIds;
				if(is_null($type->hierarchyTypeIds) == false) $hierarchyTypes = $type->hierarchyTypeIds;
			}
		...
		protected function analyzeFields() {
			...
			$orderFields = array();
			foreach($selectorOrderFields as $field) {
				if ($field instanceof selectorOrderFieldProp) {
					$orderFields[] = $field->fieldId;
				}
			}
		...
		protected function buildQuery($mode) {
			...
			if($mode == 'result') {
				...
				if (($this->selector->mode == 'objects')) {
					$this->queryColumns = $distinct ? array('COUNT(DISTINCT o.id)') : array('COUNT(o.id)');
				} else {
					$this->queryColumns = $distinct ? array('COUNT(DISTINCT h.id)') : array('COUNT(h.id)');
				}
			}
...
SQL;
		...
		protected function buildLeftJoins() {
			$joins = array();
			$data_joins = array_merge($this->selector->orderFieldProps, $this->selector->whereFieldProps, $this->selector->groupFieldProps);
			$fieldsId = array();
		...
		protected function buildTables() {
			...
			foreach($this->queryTables as $alias => $name) {
				...
				if ( (strpos($name, 'cms3_object_content') === 0) && ($alias != 'o_asteriks') ) continue;
				$tables[] = $name . ' ' . $alias;
			}
		...
		protected function buildWhere() {
			...
			foreach($this->selector->types as $type) {
				if(is_null($type->objectTypeIds) == false) $objectTypes = array_merge($type->objectTypeIds, $objectTypes);
				if(is_null($type->hierarchyTypeIds) == false) $hierarchyTypes = array_merge($type->hierarchyTypeIds, $hierarchyTypes);
			}

			$this->hierarchyTypesCount = count($hierarchyTypes);
			...
			if(sizeof($hierarchyTypes) && $this->selector->mode == 'pages') {
				$hierarchyTypes = array_unique($hierarchyTypes);
				$conds[] = 'h.type_id IN (' . implode(', ', $hierarchyTypes) . ')';
			}

			if(sizeof($hierarchyTypes) && $this->selector->mode == 'objects') {
				$hierarchyTypes = array_unique($hierarchyTypes);
				$conds[] = 't.hierarchy_type_id IN (' . implode(', ', $hierarchyTypes) . ')';
				$conds[] = 't.id = o.type_id';
			}

			if ($this->selector->mode == 'objects' && !array_search('t.id = o.type_id', $conds)) {
				$conds[] = 't.id = o.type_id';
			}

			if(sizeof($this->queryFields || sizeof($this->orderFields))) {
				$this->requireTable('o', 'cms3_objects');
			}
			...
			foreach($sysProps as $sysProp) {
				$cond = $this->buildSysProp($sysProp);
				if (!$cond) {
					continue;
				}

				if (isset($or_mode->value['field']) && in_array($sysProp->name, $or_mode->value['field'])) {
					$sys_where_conds[$sysProp->name][] = $cond;
				} else {
					$conds[$sysProp->name] = $cond;
				}
			}
			...
			if (count($sys_where_conds) > 0) {
				foreach ($sys_where_conds as $sys_where_cond) {
				   if (is_array($sys_where_cond) || count($sys_where_cond) > 1) {
					   $sql .= ' AND (' . implode(' OR ', $sys_where_cond) . ')';
				   }
				}
			}

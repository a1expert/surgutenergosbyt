	...
	class xmlImporter implements iXmlImporter {
		...
		*/
		...
		public function loadXmlString($xml_string) {
			return secure_load_dom_document($xml_string, $this->doc);
		}
		...
		public function loadXmlFile($xml_filepath) {
			...
			return secure_load_dom_document(file_get_contents($xml_filepath), $this->doc);
		}
		...
		*/
		...
		protected function detectTemplateId($filepath, $domain_id = false, $lang_id = false) {
			if($filepath) {
				...
				foreach($templates as $ctpl) {
					if($ctpl->getFilename() == $filepath || (is_numeric($filepath) && $filepath == $ctpl->getId())) {
						return $ctpl->getId();
					}
				}
			}
		...
		protected function importHierarchyRelation(DOMElement $info) {
			$oldId = $info->getAttribute('id');
			$oldParentId = $info->getAttribute('parent-id');
		...
		protected function importHierarchy() {
			...
			if($nl->length) {
				...
				$parents = array();
				foreach ($nl as $info) {
					$parentId = $this->importHierarchyRelation($info);
					if ($parentId !== false) $parents[$parentId] = $parentId;
				}
				
				foreach($parents as $parentId) {
					umiHierarchy::getInstance()->rebuildRelationNodes($parentId);	
				}
			}
		}

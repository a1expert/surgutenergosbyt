<?php
	class xmlExporter implements iXmlExporter {
		...
		public function excludeBranches(array $branches) {
			foreach ($branches as $elementId) {
				if ($elementId instanceof umiHierarchyElement) $elementId = $elementId->getId();
		...
		public function addTemplates($templates = array()) {
			foreach ($templates as $template) {
				if($template instanceof template) $template = $template->getId();
		...
		protected function createGrid() {
			...
			$n->appendChild($d->createCDATASection(def_module::parseTPLMacroses(macros_sitename())));
			$m->appendChild($n);
		...
		protected function exportType($type_id) {
			...
			if($parser->evaluate("fieldgroups/group", $c)->length) {
				foreach($parser->evaluate("fieldgroups/group", $c) as $group) {
					...
					if (!$typeGroup instanceof umiFieldsGroup) {
						continue;
					}
			...
			foreach ($nl as $field) {
				...
				if ($guide_id && (!$this->ignoreRelations || in_array('guides', $this->saveRelations))) {
					...
					foreach ($sel->result as $res) {
						if (is_array($this->objects) && !in_array($res['id'], $this->objects)) {
							$this->objects[] = $res['id'];
						}
					}
		...
		protected function exportBranch($element_id) {
			if (isset($this->excludedElements[$element_id])) return false;

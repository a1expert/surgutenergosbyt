<?php
	abstract class __editor_content {
		...
		public function frontendPanel() {
			...
			$tickets->types('hierarchy-type')->name('content', 'ticket');
		...
		public function getIeditorImagesPath ($bFullPath = false) {
			$sPath = $bFullPath ? CURRENT_WORKING_DIR : '';
			return $sPath . '/images/cms/data/.ieditor';
		}
		...
		public function isThumb ($sImagePath) {
			return strpos($sImagePath, '/images/cms/autothumbs/') !== false || strpos($sImagePath, '/images/cms/thumbs/') !== false;
		}
		...
		public function generateOriginalImagePath ($arPathInfo, $arAdditionalInfo = array()) {
			$sIeditorImagesFolder = CURRENT_WORKING_DIR . $this->getIeditorImagesPath();
		...
		public function findOriginalImages ($sImagePath) {
			...
			$sSearchString = CURRENT_WORKING_DIR . $this->getIeditorImagesPath() . '/' . $sSearchString . "*." . $arPathInfo['extension'];
		...
		public function deleteThumbnail ($sImagePath) {
			@unlink(CURRENT_WORKING_DIR . '/images/cms/data/.tmb/' . md5($sImagePath) . '.png');
		}
		...
		public function ieditor_resize ($sImagePath){
			...
			try {
				...
				if (!$oImagemagic->resize($sImagePath, $iWidth, $iHeight)) {
					throw new Exception('Failed to resize image by ImageMagic');
				}
		...
		/**
		 * Get imagemagic instance
		 * @return imagemagick
		 * @throws Exception
		 */
		public function getImageMagic () {
			if(!method_exists('regedit', 'onController') || !regedit::onController()) {
				throw new Exception('Not on controller');
			}
			require_once(CURRENT_WORKING_DIR . "/../../../../umihost/area_init.php");
			if (!method_exists('umihost_config', 'get')) {
				throw new Exception('Failed to load controller environment');
			}
			set_time_limit(120);

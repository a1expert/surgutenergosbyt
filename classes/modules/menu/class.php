...
class menu extends def_module {
	...
	public function draw($menuId = NULL, $template = "default") {
		...
		if (!is_numeric($menuId)) {
			...
			$selector->types('hierarchy-type')->name('menu', 'item_element');
		...
		$this->loadElements($menuHierarchyArray);
	...
	protected function loadElements(array $menuHierarchyArray) {
		...
		if(is_array($values) || is_object($values)){
			...
			foreach($values as $key => $value){
				...
				if($rel!='custom' && $rel!='system'){
					...
					if(!$link) {unset($values[$key]); continue;}
					$value->link = $link;
	...
	*/
	public function getListMenu(){
		...
		$sel->types('hierarchy-type')->name('menu', 'item_element');

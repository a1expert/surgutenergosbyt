<?php
	abstract class __dispatches extends baseModuleAdmin {
		...
		public function lists() {
			...
			$sel->types('hierarchy-type')->name('dispatches', 'dispatch');
		...
		public function releases() {
			...
			$sel = new selector('objects');
			$sel->types('hierarchy-type')->name('dispatches', 'release');
		...
		public function onModifyObject($e) {
			...
			$subscriberTypeId = umiObjectTypesCollection::getInstance()->getTypeByHierarchyTypeId($subscriberHierarchyTypeId);
			if($subscriberTypeId == $object->getTypeId()) {
				if (!umiMail::checkEmail((string) getRequest('name'))) {

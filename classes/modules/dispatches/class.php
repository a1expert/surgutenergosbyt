<?php
	class dispatches extends def_module {
		...
		public function getAllDispatches() {
			...
			$dispatches->types('hierarchy-type')->name('dispatches', 'dispatch');
		...
		public function subscribe_do() {
			...
			if($oMdlUsers = $controller->getModule("users")) {
				if($oMdlUsers->is_auth()) {
					...
					$dispatches = array();
					foreach ($arrSbsDispatches as $dispatch) {
						if (!$dispatch instanceof umiObject) {
							continue;
						}
						$dispatches[] = $dispatch->getId();
					}
					$arrSbsDispatches = $dispatches;
				}
				else {
					$sResult = "%subscribe_incorrect_email%";
					return (!def_module::isXSLTResultMode()) ? $sResult : Array("result" => array("@class"=>"error","node"=>$sResult));
				}

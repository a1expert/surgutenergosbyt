<?php
	class comments extends def_module {
		...
		public function countComments($parent_element_id = false) {
			...
			$comments->types('hierarchy-type')->name('comments', 'comment');
			$comments->where('hierarchy')->page($parent_element_id)->childs(1);
		...
		public function comment() {
			...
			$comments = new selector('pages');
			$comments->types('hierarchy-type')->name('comments', 'comment');
			$comments->where('hierarchy')->page($parent_id)->childs(1);

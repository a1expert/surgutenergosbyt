...
class webforms extends def_module {
	...
	public function guessAddressId($_sAddress) {
		...
		$addresses->types('hierarchy-type')->name('webforms', 'address');
	...
	private function writeAddressSelect($template, $iFormId = false) {
		...
		foreach($addresses as $address) {
			...
			$aParam  = array();
			$aParam['attribute:id'] = $address->getId();
	...
	public function getAddresses($iFormId = false) {
		...
		$sel->types('hierarchy-type')->name('webforms', 'address');
		$result = $sel->result();
		$aBlock = Array();
		$aLines = Array();
...
END;
	...
	public function posted($template = false) {
		...
		if ($template && is_numeric($template)) {
			...
			$sel = new selector('objects');
			$sel->types('hierarchy-type')->name('webforms', 'template');

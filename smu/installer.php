...
/*					// восстановление из бекапа файлов
					...
					case 'get-solution-list':    $result = $this->getDemositeList(); break;
		...
		*/
		private function installDemosite() {
			...
			do {
				...
				if ($offset==0) {
					$this->flushLog("Установка сайта {$name}...");
				}
			...
			$this->flushLog("Сайт {$name} установлен.");
	...
	class umiUpdateDownloader {
		...
		*/
		...
		public function getDemositesList() {
			$result = $this->query('get-solution-list');
			$doc = new DOMDocument('1.0', 'utf-8');
		...
		*/
		public function extractFiles($offset = false, $limit = false) {
            ...
            if (extension_loaded('phar')) {
                $pathList = mb_substr($this->archiveFilename, 0, (strlen($this->archiveFilename)-4));
